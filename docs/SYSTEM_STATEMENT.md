...
현재 버전 (2026-06-05 기준): 앱 `1.0.15`, 위젯 `V.2026.6.8`.

> 앱(Electron 셸 설치본)은 `1.0.15` 부터 정식 릴리즈 — `1.0.X` semver 로 관리한다. (`0.2.X` 는 정식 릴리즈 전 베타 버전대. 패치 번호 X 는 베타 시절 카운트를 이어서 증가 — 0.2.14 다음이 1.0.15)
